...
public class ServiceProviderCore {
    ...
    public ServiceProviderCore(String providerAddress, String serviceRegistryAddress) {
        if(!StringUtil.checkAddress(providerAddress) || !StringUtil.checkAddress(serviceRegistryAddress)){
            logger.error("address format error");
            return;
        }
        ...
        providerWorkThread = new Thread(() -> {
            ...
            try {
                ...
                if (e instanceof InterruptedException) {
                    logger.info("Rpc server remoting server interrupted");
                } else {
